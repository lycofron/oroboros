...
class FilterDialog(QDialog):
    ...
    def ephePathSelect(self):
        ...
        if self.ephetypeEdit.currentIndex() == 0:  # swisseph
            ...
            QMessageBox.information(self, self.tr('Moshier ephemeris'), self.tr(
                'No ephemeris path needed for Moshier ephemeris.'))
        if path != '':
    ...
    def accept(self):
        ...
        # aspects restrictions
        asprestr = all_aspects_restrictions_names()[
            self.asprestrEdit.currentIndex()]
        # orbs restrictions
        orbrestr = all_orbs_restrictions_names()[
            self.orbrestrEdit.currentIndex()]
        # midpoints settings
        calc_midp = self.calc_midpEdit.isChecked()
        draw_midp = self.draw_midpEdit.isChecked()
        # midpoints filter
        midpoints = all_midpoints_filters_names()[
            self.midpointsEdit.currentIndex()]
        # comment
