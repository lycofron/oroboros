...
class MidPointsFilterDialog(QDialog):
    ...
    def __init__(self, parent=None, filt=None):
        ...
        if isinstance(filt, MidPointsFilter):
            self._filt = filt
            title = unicode(
                tr('Edit MidPoints Filter \xab %(filter)s \xbb')) % {
                'filter': filt._name}
        else:
