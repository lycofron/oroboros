...
class AspectsRestrictionsDialog(QDialog):
    ...
    def __init__(self, parent=None, filt=None):
        ...
        if isinstance(filt, AspectsRestrictions):
            self._filt = filt
            title = unicode(
                tr('Edit Aspects Restrictions \xab %(filter)s \xbb')) % {
                'filter': filt._name}
        else:
        ...
        # lilith (mean)
        self._cb['Lilith (mean)'] = QCheckBox(
            names.planets['Lilith (mean)'], self)
        mainGrid.addWidget(self._cb['Lilith (mean)'], 4, 1)
        # lilith (true)
        self._cb['Lilith (true)'] = QCheckBox(
            names.planets['Lilith (true)'], self)
        mainGrid.addWidget(self._cb['Lilith (true)'], 4, 2)
        # priapus (mean)
        self._cb['Priapus (mean)'] = QCheckBox(
            names.planets['Priapus (mean)'], self)
        mainGrid.addWidget(self._cb['Priapus (mean)'], 4, 3)
        # priapus (true)
        self._cb['Priapus (true)'] = QCheckBox(
            names.planets['Priapus (true)'], self)
        ...
        # neptune (leverrier)
        self._cb['Neptune (Leverrier)'] = QCheckBox(
            names.planets['Neptune (Leverrier)'], self)
        othersGrid.addWidget(self._cb['Neptune (Leverrier)'], 0, 3)
        # neptune (adams)
        self._cb['Neptune (Adams)'] = QCheckBox(
            names.planets['Neptune (Adams)'], self)
        othersGrid.addWidget(self._cb['Neptune (Adams)'], 1, 0)
        # pluto (lowell)
        self._cb['Pluto (Lowell)'] = QCheckBox(
            names.planets['Pluto (Lowell)'], self)
        othersGrid.addWidget(self._cb['Pluto (Lowell)'], 1, 1)
        # pluto (pickering)
        self._cb['Pluto (Pickering)'] = QCheckBox(
            names.planets['Pluto (Pickering)'], self)
        ...
        # equatorial asc
        self._cb['Equatorial Ascendant'] = QCheckBox(
            names.planets['Equatorial Ascendant'], self)
        cuspsGrid.addWidget(self._cb['Equatorial Ascendant'], 4, 2)
        # co-ascendant (koch)
        self._cb[
            'Co-ascendant (Koch)'] = QCheckBox(names.planets['Co-ascendant (Koch)'], self)
        cuspsGrid.addWidget(self._cb['Co-ascendant (Koch)'], 4, 3)
        # co-ascendant (munkasey)
        self._cb['Co-ascendant (Munkasey)'] = QCheckBox(
            names.planets['Co-ascendant (Munkasey)'], self)
        cuspsGrid.addWidget(self._cb['Co-ascendant (Munkasey)'], 5, 0)
        # polar ascendant
        self._cb['Polar Ascendant (Munkasey)'] = QCheckBox(
            names.planets['Polar Ascendant (Munkasey)'], self)
    ...
    def accept(self):
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                tr('Error'),
                unicode(
                    tr('Duplicate filter name \xab %(filter)s \xbb.')) % {
