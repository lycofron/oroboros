...
def createActions(self):
    ...
    self.actionAspectsRestrictions.setToolTip(
        tr('Manage Aspects Restrictions'))
    self.actionAspectsRestrictions.setStatusTip(
        tr('Manage Aspects Restrictions'))
    ...
    self.actionProgressionMode = QAction(
        ...
        tr('Progression'), self)
    self.actionProgressionMode.setShortcut(
        tr('Ctrl+P', 'Progression Shortcut'))
    ...
    self.actionMidSpaceTime.setShortcut(
        tr('Ctrl+X', 'Mid-space/time shortcut'))
...
def reset(self, app):
    ...
    if len(app.desktop.charts) == 0:
        ...
        if len(
            app.desktop.charts[
                self.central.currentIndex()]) > 1:  # two subcharts
