...
class MainWindow(QMainWindow):
    ...
    def aboutOroborosEvent(self):
        msg = unicode(self.tr(
            """<qt>This is <b><i>Oroboros</i></b> astrology software (version %(version)s).<br><br>
...
FITNESS FOR A PARTICULAR PURPOSE.</qt>""")) % {'version': oroboros.__version__}
    ...
    def hideChart1Event(self):
        idx = self.central.currentIndex()
        app.desktop.charts[idx][
            0]._hidden = not app.desktop.charts[idx][0]._hidden
        self.central.resetTab(idx)

    def hideChart2Event(self):
        idx = self.central.currentIndex()
        app.desktop.charts[idx][
            1]._hidden = not app.desktop.charts[idx][1]._hidden
        self.central.resetTab(idx)
