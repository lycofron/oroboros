...
class Element(object):
    ...
    def __init__(self, tag=None, attributes={}, text='', tail='',
        ...
        """
        if _etree_elem is not None:  # got etree element
            self._elem = _etree_elem
            return
        if tag is None:
            tag = _default_tag
        self._elem = etree.Element(_default_tag)
        self.tag, self.attributes, self.text, self.tail = (
            tag, attributes, text, tail)
    ...
    def get_child(self, idx=0, tag=None):
        ...
        if tag is None:  # search all tags
    ...
    def get_iterator(self, tag):
        ...
        :rtype: iterator
        """
        return (Element(_etree_elem=x)
                for x in self._elem.findall(_serialize(tag)))
