...
class Planet(db.Object):
    ...
    def set(self, _idx=None, num=None, name=None, family=None, ranking=None,
        ...
        if bool_use is not None:
            self.bool_use = bool_use
        if bool_aspect is not None:
            self.bool_aspect = bool_aspect
        if default_orbmod is not None:
    ...
    def _from_xml(self, elem, with_idx=False):
        ...
        if with_idx:
            ...
            if _idx == 'None':
                ...
                self._idx = _idx
        self.set(
            num=elem.get_child_text(
                tag='num'),
            name=elem.get_child_text(
                tag='name').replace(
                '&lt;',
                '<').replace(
                    '&gt;',
                    '>'),
            family=elem.get_child_text(
                tag='family'),
            ranking=elem.get_child_text(
                tag='ranking'),
            bool_use=elem.get_child_text(
                tag='bool_use'),
            bool_aspect=elem.get_child_text(
                tag='bool_aspect'),
            default_orbmod=elem.get_child_text(
                tag='default_orbmod'),
            glyph=elem.get_child_text(
                tag='gylph'),
