...
class ChartCalc(ChartDate):
    ...
    __slots__ = ChartDate.__slots__ + ['_ecl_nut',
                                       '_planets',
                                       '_houses',
                                       '_aspects',
                                       '_midpoints',
    ...
    def _calc_houses(self):
        ...
        self._setup_swisseph()
        cusps, ascmc = swe.houses_ex(self.julday, float(self._latitude), float(
            self._longitude), self._filter._hsys, self._filter.get_calcflag())
            ...
            for pos2 in all_planets[i + 1:]:
                ...
                for asp, doasp in f._aspects.items():
                    ...
                    if orb < 0:  # we'll never get such a precision
                        continue
                    # check aspect match
                    diff, application, factor = swe._match_aspect2(
                        lon1, lonsp1, lon2, lonsp2,
                        float(asp._angle), float(orb))
                    if diff is not None:
                        res.feed(pos1, pos2, asp, diff, application, factor)
        self._aspects = res
                ...
                res.feed(pos1, pos2,
                         (swe.deg_midp(pos1._longitude, pos2._longitude),  # midp long
                          (pos1._latitude + pos2._latitude) / 2,  # midp lat
                             ...
                             (pos1._lonspeed + pos2._lonspeed) / \
                          2,  # midp long speed
                             (pos1._latspeed + pos2._latspeed) / \
                          2,  # midp lat speed
    ...
    # comparisons (transform planets positions relatively to another julian
    # day)
